<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="FixtureList.aspx.cs" Inherits="PE2.Fixture.FixtureList" %>
...
<script type="text/javascript">
    ...
    function OpenDialog() {
        if (win1) {
            ...
            win1 = $.ligerDialog.open({ height: 420, url: 'FixtureSearch.aspx?M=List', width: 550, title: '' });
        }

    }
    function OpenEditDialog(val) {
        if (edit) {
            edit = $.ligerDialog.open({ height: 600, url: 'FixtureEdit.aspx?CP=0&Mode=' + val, width: 800, title: '' });
        }
        else {
            edit = $.ligerDialog.open({ height: 600, url: 'FixtureEdit.aspx?CP=0&Mode=' + val, width: 800, title: '' });
        }
    ...
    function OpenCopyDialog(val) {
        if (edit) {
            edit = $.ligerDialog.open({ height: 600, url: 'FixtureEdit.aspx?CP=1&Mode=' + val, width: 800, title: '' });
        }
        else {
            edit = $.ligerDialog.open({ height: 600, url: 'FixtureEdit.aspx?CP=1&Mode=' + val, width: 800, title: '' });
        }
    ...
    function IsRemove(val) {
        $.ligerDialog.prompt("確定報廢此治具嗎?", function (yes, value) {
            if (yes) {
                $.ajax({
                    url: '/PE2/Ashx/ChangeRemove.ashx',
                    data: { Fixture_sn: val, Note: value },
                    type: 'GET',
    ...
    function IsChange(val) {
        $.ligerDialog.prompt("確定轉出此治具嗎?", function (yes, value) {
            if (yes) {
                $.ajax({
                    url: '/PE2/Ashx/ChangeOther.ashx',
                    data: { Fixture_sn: val, Note: value },
                    type: 'GET',
                    contentType: 'application/json; charset=utf-8',
                    dataType: 'json',
    ...
    <asp:GridView ID="gvFixture" runat="server" CellPadding="3" CssClass="EU_DataTable"
        ...
        <Columns>
            <asp:TemplateField HeaderText="功能">
                ...
                <ItemTemplate>
                    <asp:LinkButton ID="LinkButton2" runat="server" CommandName="RowDelete" CommandArgument='<%# Eval("Fixture_no") %>'
                        OnClientClick='<%# @"return OpenCopyDialog("""+ Eval("Fixture_sn") + @""");" %>'>
                        複製</asp:LinkButton>
                    ...
                    <asp:LinkButton ID="btnRemove" runat="server" CommandName="RowDelete" CommandArgument='<%# Eval("Fixture_no") %>'
                        ...
                        報廢</asp:LinkButton>
                    <asp:LinkButton ID="btnChange" runat="server" CommandName="RowDelete" CommandArgument='<%# Eval("Fixture_no") %>'
                        OnClientClick='<%# @"return IsChange("""+ Eval("Fixture_sn") + @""");" %>'>
                        轉出</asp:LinkButton>
           ...
           <asp:BoundField DataField="Place_name" HeaderText="目前位置">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            ...
            <asp:BoundField DataField="Out_Place_name" HeaderText="出借位置">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            <asp:BoundField DataField="Out_User_name" HeaderText="出借人員">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            <asp:BoundField DataField="Out_Note" HeaderText="出借備註">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            
            <asp:BoundField DataField="In_date" HeaderText="歸還日期" DataFormatString="{0:yyyy/MM/dd}" >
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
             <asp:BoundField DataField="In_Place_name" HeaderText="歸還位置">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            <asp:BoundField DataField="In_User_name" HeaderText="歸還人員">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
            <asp:BoundField DataField="In_Note" HeaderText="歸還備註">
                <HeaderStyle Wrap="False" />
                <ItemStyle Wrap="False" />
            </asp:BoundField>
        </Columns>
