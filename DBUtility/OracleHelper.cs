                    ...
                    foreach (CommandInfo c in cmdList)
                    {
                        if (!String.IsNullOrEmpty(c.CommandText))
                        {
                            ...
                            if (c.EffentNextType == EffentNextType.WhenHaveContine || c.EffentNextType == EffentNextType.WhenNoHaveContine)
                            {
                                ...
                                {
                                    tx.Rollback();
                                    throw new Exception("Oracle:峊掖猁" + c.CommandText + "斛剕睫磁select count(..腔跡宒");
                                    //return false;
                                }
                                ...
                                {
                                    tx.Rollback();
                                    throw new Exception("Oracle:峊掖猁" + c.CommandText + "殿隙硉斛剕湮衾0");
                                    //return false;
                                }
                                if (c.EffentNextType == EffentNextType.WhenNoHaveContine && isHave)
                                {
                                    tx.Rollback();
                                    throw new Exception("Oracle:峊掖猁" + c.CommandText + "殿隙硉斛剕脹衾0");
                                    //eturn false;
                                }
                                continue;
                            }
                            ...
                            {
                                tx.Rollback();
                                throw new Exception("Oracle:峊掖猁" + c.CommandText + "斛剕衄荌砉俴");
                                // return false;
                            }
                        }
                    }
